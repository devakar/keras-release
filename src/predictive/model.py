...
log_dir = "stateful-2-32-LeakyReLU-scale-epoch10-multi-stateful"
...
checkpoint = ModelCheckpoint("res-10epoch-scaled-17may-check.h5", monitor='loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
...
def create_model():
    ...
    model.add(LSTM(32, stateful=True, name='layer-2'))
    model.add(LeakyReLU(alpha=.001))
    model.add(Dense(5, name='output-layer'))
    model.compile(loss='mse', optimizer='adam', metrics=['mse', 'mae'])
    for i in range(10):
        print("Number of iteration", i+1)
        model.fit(train_X, train_y, epochs=1, batch_size=1, verbose=2, shuffle=False, validation_data=(train_X, train_y), callbacks=[reduce_lr, early_stopping, logger, csv_logger, checkpoint])
        model.reset_states()
    model.save("res-10epoch-scaled-17may.h5")

def models():
    print("Call to model bulilding")
    time.sleep(300)
    print("call finished")
